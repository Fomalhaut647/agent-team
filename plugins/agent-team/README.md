...
Two subskills for coordinating long-running Claude Code agent teams, plus optional reference docs for layering the `superpowers` development workflow on top.
...
- **Optional reference docs** under each skill's `references/` directory cover the full `superpowers` development workflow (brainstorming → writing-plans → TDD implementers → reviewer teammates → PR fix loop → merge). Read on demand from the lead skill's Step 3 decision; spawn prompts ask teammates to read their corresponding doc when the workflow signals it.
...
```
/plugin install <repo-url-or-local-path>
```
